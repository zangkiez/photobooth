...
var remoteBuzzerClient;
var rotaryController;
...
var checkRemoteBuzzerReachable = function checkRemoteBuzzerReachable(baseUrl) {
...
function initRemoteBuzzerFromDOM() {
  photoboothTools.console.logDev(config.remotebuzzer.usebuttons || config.remotebuzzer.userotary ? 'Remote buzzer server is enabled.' : 'Remote buzzer server is disabled.');
  ...
  remoteBuzzerClient = function () {
    ...
    api.init = function () {
      ...
      if (config.remotebuzzer.serverip) {
        var baseUrl = window.location.protocol + '//' + config.remotebuzzer.serverip + ':' + config.remotebuzzer.port;
        checkRemoteBuzzerReachable(baseUrl).then(function (reachable) {
          if (!reachable) {
            photoboothTools.console.log('Remote buzzer server not reachable at ' + baseUrl + ' - skipping connection.');
            return;
          }
          photoboothTools.console.log('Remote buzzer server is reachable at ' + baseUrl + ' - skipping connection.');
          ...
          ioClient.on('connect_error', function () {
            photoboothTools.console.log('ERROR: Remote buzzer client unable to connect to Remote buzzer Server - please ensure Remote buzzer server is running on ' + config.remotebuzzer.serverip + '. Set Photobooth loglevel to 1 (or above) to create log file for debugging.');
          });
          ioClient.on('connect', function () {
            photoboothTools.console.logDev('Remote buzzer client successfully connected to Remote buzzer Server.');
          });
        ...
        photoboothTools.console.log('ERROR: Remote buzzer client unable to connect - Remote buzzer Server IP not defined in photobooth config!');
      }
    };
  ...
  rotaryController = function () {
    ...
    // API functions
    api.enabled = function () {
      return config.remotebuzzer.userotary && (typeof onStandaloneGalleryView === 'undefined' ? true : config.remotebuzzer.enable_standalonegallery);
    };
    api.init = function () {
      if (config.dev.loglevel > 0 && typeof onStandaloneGalleryView !== 'undefined') {
        photoboothTools.console.log('Rotary controller is ', config.remotebuzzer.enable_standalonegallery ? 'enabled' : 'disabled', ' for standalone gallery view.');
      }
    };
    ...
    api.focusNext = function () {
      ...
      if ($('.pswp.pswp--open').is(':visible')) {
        ...
        var buttonList = $('.pswp.pswp--open').find('.rotaryfocus:visible');
        var focusIndex = buttonList.index($('.focused'));
        if (buttonList.eq(focusIndex + 1).exists()) {
          focusIndex += 1;
        } else if (buttonList.eq(0).exists()) {
          focusIndex = 0;
        }
        $('.focused').removeClass('focused pswp-rotary-focus').parents('.pswp.pswp--open').find('.rotaryfocus:visible').eq(focusIndex).addClass('focused pswp-rotary-focus').find('i.fa').css('z-index', '1');
      } else {
        var _buttonList = $('.focused').parents('.rotarygroup').find('.rotaryfocus:visible');
        var _focusIndex = _buttonList.index($('.focused'));
        if (_buttonList.eq(_focusIndex + 1).exists()) {
          _focusIndex += 1;
        } else if (_buttonList.eq(0).exists()) {
          _focusIndex = 0;
        }
        $('.focused').removeClass('focused').parents('.rotarygroup').find('.rotaryfocus:visible').eq(_focusIndex).addClass('focused').focus();
      }
    };
    api.focusPrev = function () {
      ...
      if ($('.pswp.pswp--open').is(':visible')) {
        ...
        var focusIndex = buttonList.index($('.focused'));
        if (buttonList.eq(focusIndex - 1).exists()) {
          $('.focused').removeClass('focused pswp-rotary-focus').parents('.pswp.pswp--open').find('.rotaryfocus:visible').eq(focusIndex - 1).addClass('focused pswp-rotary-focus').find('i.fa').css('z-index', '1');
        }
      } else {
        var _buttonList2 = $('.focused').parents('.rotarygroup').find('.rotaryfocus:visible');
        var _focusIndex2 = _buttonList2.index($('.focused'));
        if (_buttonList2.eq(_focusIndex2 - 1).exists()) {
          $('.focused').removeClass('focused').parents('.rotarygroup').find('.rotaryfocus:visible').eq(_focusIndex2 - 1).addClass('focused').focus();
        }
      }
    };

...
function initPhotoSwipeFromDOM(gallerySelector) {
  ...
  var openPhotoSwipe = function openPhotoSwipe(selector, galIndex) {
    ...
    gallery.on('uiRegister', function () {
      ...
      if (config.download.enabled) {
        gallery.pswp.ui.registerElement({
          ...
          onInit: function onInit(el, pswp) {
            pswp.on('change', function () {
              var src = pswp.currSlide.data.src || '';
              var basename = src.split('\\').pop().split('/').pop().split('?')[0];
              var url = environment.publicFolders.api + '/download.php?image=' + encodeURIComponent(basename);
    ...
    gallery.on('contentDeactivate', function (_ref2) {

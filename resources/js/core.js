...
var photoBooth = function () {
  ...
  api.init = function () {
    ...
    if (!stageGuardObserver && loader.length) {
      stageGuardObserver = new MutationObserver(function () {
        if (loader.hasClass('stage--active') && !api.takingPic && !api.isFilterProcessing && !resultPage.hasClass('stage--active')) {
          ensureIdleStage();
        }
      });
  ...
  api.renderPic = function (filename, files) {
      ...
      if (!captureBasename) {
        ...
        galimages.find('a').each(function () {
          ...
          if (origHref.split('/').pop().split('?')[0] === filename) {
            ...
            var img = a.find('img');
            if (img.length) {
              var thumbBase = (config.gallery.use_thumb ? environment.publicFolders.thumbs : environment.publicFolders.images) + '/' + filename;
    ...
    preloadImage.onload = function () {
      resultPage.css({
        '--stage-background-image': "url(".concat(imageUrl).concat(imageCacheKey, "&filter=").concat(imgFilter, ")")
      });
      ...
      if (config.qr.enabled && config.qr.result != 'hidden') {
        ...
        qrResultImage.src = environment.publicFolders.api + '/qrcode.php?multiple=' + encodeURIComponent(qrSessionList.join(','));
  ...
  api.addVideoToGallery = function (videoName) {
    ...
    var linkEl = $('<a>').addClass('gallery-list-item gallery-slideshow-video rotaryfocus').attr('href', videoUrl).attr('data-pswp-type', 'video-slide').attr('data-video-src', videoUrl).attr('data-video-poster', posterUrl).attr('data-pswp-width', 1280).attr('data-pswp-height', 720).attr('title', 'Slideshow MP4').append(videoEl);
    if (config.gallery.newest_first) {

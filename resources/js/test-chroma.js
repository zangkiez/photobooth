...
var photoboothChromaTest = function () {
  var api = {};
  api.runCmd = function (chromaImage, chromaColor, chromaSensitivity, chromaBlend) {
    ...
    jQuery.post('../api/liveChromaConfig.php', dataChroma).done(function () {
  ...
  $('.setChroma').on('click', function (e) {
    ...
    api.runCmd($('#chromaImage').val(), $('#chromaColor').val().replace(/#/g, ''), $('#chromaSensitivity').val(), $('#chromaBlend').val());
  });
  return api;
}();